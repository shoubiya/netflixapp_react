...
export default function SerOnTheAir() {
  const [open, setOpen] = React.useState(false);
  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);
    const[seronair,setSerontheair]=useState([])
    ...
    useEffect(()=>{
      axios.get(`https://api.themoviedb.org/3/tv/on_the_air?api_key=6cdf1bc39c14d06f4b51e70d622928f4`).then(response=>{
       console.log("response=>",response.data.results);
       setSerontheair(response.data.results)
      })
    },[])
    console.log(seronair);
    const singleview=(id)=>{
      console.log(id)
      axios.get(`https://api.themoviedb.org/3/movie/${id}?api_key=6cdf1bc39c14d06f4b51e70d622928f4`).then(response=>{
        ...
        axios.get(`https://api.themoviedb.org/3/tv/${id}/videos?api_key=6cdf1bc39c14d06f4b51e70d622928f4`).then(response=>{
    ...
    return (
      <div>
      <h1 className='head1'>OnTheAir Series</h1>
      <div className='slide1'>
        ...
        <div className='slide-items'>
          ...
          <img className='slides'onClick={()=>{handleOpen();singleview(OnThe?.id)}} src={`https://image.tmdb.org/t/p/original${OnThe.poster_path}`} alt=''></img>
          </div>
          </>
           ))}
        </div>
