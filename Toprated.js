...
export default function Toprated() {
  ...
  const handleClose = () => setOpen(false);
    ...
    useEffect(()=>{
      axios.get(`https://api.themoviedb.org/3/movie/top_rated?api_key=6cdf1bc39c14d06f4b51e70d622928f4`).then(response=>{
    ...
    return (
      <div>
      <h1 className='head14'>TopRated Movies</h1>
      <div className='slide1'>
        ...
        <div className='slide-items'>
          ...
          <img className='slides'onClick={()=>{handleOpen();singleview(top?.id)}} src={`https://image.tmdb.org/t/p/original${top.poster_path}`} alt=''></img>
          </div>
          </>
           ))}
        </div>
