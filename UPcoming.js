...
export default function UPcoming() {
  ...
  const handleClose = () => setOpen(false);
    ...
    useEffect(()=>{
      axios.get(`https://api.themoviedb.org/3/movie/upcoming?api_key=6cdf1bc39c14d06f4b51e70d622928f4`).then(response=>{
       console.log("response=>",response.data.results);
       setUpcome(response.data.results)
      })
    },[])
    console.log(Upcome);
    ...
    return (
      <div>
      <h1 className='head15'>Upcoming Movies</h1>
      <div className='slide1'>
        ...
        <div className='slide-items'>
          ...
          <img className='slides' onClick={()=>{handleOpen();singleview(upcom?.id)}}  src={`https://image.tmdb.org/t/p/original${upcom.poster_path}`} alt=''></img>
          </div>
          </>
           ))}
        </div>
