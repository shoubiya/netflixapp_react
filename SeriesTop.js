...
export default function SeriesTop() {
  ...
  const handleClose = () => setOpen(false);
    ...
    useEffect(()=>{
      axios.get(`https://api.themoviedb.org/3/tv/top_rated?api_key=6cdf1bc39c14d06f4b51e70d622928f4`).then(response=>{
       console.log("response=>",response.data.results);
       setSeriestop(response.data.results)
      })
    },[])
    ...
    return (
      <div>
      <h1 className='head1'>Top Rated Series</h1>
      <div className='slide1'>
        ...
        <div className='slide-items'>
          ...
          <img className='slides' onClick={()=>{handleOpen();singleview(tops?.id)}} src={`https://image.tmdb.org/t/p/original${tops.poster_path}`} alt=''></img>
          </div>
          </>
           ))}
        </div>
